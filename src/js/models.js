...
function getSolarSystemModels(gl) {
  const sun = { bufferInfo: flattenedPrimitives.createSphereBufferInfo(gl, 10 * 10, 10 * 80, 30), color: [0.9921, 0.7226, 0.07421, 1], translation: [0, 0, 0]};
  ...
  const jupiter = { bufferInfo: flattenedPrimitives.createSphereBufferInfo(gl, 0.90909 * 80, 0.90909 * 20, 30), color: [0.8398, 0.7929, 0.6171, 1], translation: [778.6 / scale, 0, 0] }; // 11
  const saturn = { bufferInfo: flattenedPrimitives.createSphereBufferInfo(gl, 0.83333 * 80, 0.83333 * 20, 30), color: [0.9179, 0.8359, 0.7226, 1], translation: [1433.5 / scale, 0, 0] }; // 12
  const uranus = { bufferInfo: flattenedPrimitives.createSphereBufferInfo(gl, 0.36496 * 80, 0.36496 * 20, 30), color: [0.7734, 0.8242, 0.8906, 1], translation: [2872.5 / scale, 0, 0] }; // 27.4
  const neptune = { bufferInfo: flattenedPrimitives.createSphereBufferInfo(gl, 0.36101 * 80, 0.36101 * 20, 30), color: [0.2500, 0.3320, 0.7304, 1], translation: [4495.1 / scale, 0, 0] }; // 27.7
